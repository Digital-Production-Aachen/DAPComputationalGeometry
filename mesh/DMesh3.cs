...
{
    /// <summary>
    /// enum classifying the mesh for AM production
    /// </summary>
    ...
    {
        internal List<GeneralPolygon2d> cutShadowHint;
        public System.Drawing.Color? Color { get; set; }

        public int RealShadowTriangleCountLimit { get; set; } = 500000;

        //variables with cached computations, get invalidated on rotate
        private OpenVectorFormat.Part.Types.GeometryInfo _geometryInfo;
        private volatile GeneralPolygon2d[] _shadow;
        private volatile Polygon2d _convexShadow;
        private ImplicitSphere3d _boundingSphereCentroid;
        private Vector3d _centroid;
        private int convexShadowTimeStamp = -1;
        private int shadowTimeStamp = -1;
        private int geometryInfoTimeStamp = -1;
        private int geometryInfoShapeTimeStamp = -1;
        private int _centroidBoundingSphereTimeStamp = -1;
        private int _centroidTimestamp = -1;

        public string PartName { get; set; } = "Unnamed Part";
        ...
        public Vector3d Centroid
        {
            get
            {
                if (_centroidTimestamp != timestamp)
                {
                    _centroid = MeshMeasurements.Centroid(this);
                }
                return _centroid;
            }
        }
        ...
        /// <summary>
        /// Computes the bounding sphere of the mesh with the centroid as sphere center.
        /// </summary>
        /// <param name="mesh"></param>
        /// <returns></returns>
        public ImplicitSphere3d BoundingSphere()
        {
            ImplicitSphere3d boundingSphere = _boundingSphereCentroid;
            if (_centroidBoundingSphereTimeStamp != timestamp)
            {
                int timeStampSnapshot = Timestamp;
                boundingSphere = BoundingSphere(Centroid);
                _boundingSphereCentroid = boundingSphere;
                _centroidBoundingSphereTimeStamp = timeStampSnapshot;
            }
            return new ImplicitSphere3d(boundingSphere);
        }

        /// <summary>
        /// Computes the bounding sphere of the mesh with the given sphere center.
        /// </summary>
        /// <param name="mesh"></param>
        /// <param name="center"></param>
        /// <returns></returns>
        public ImplicitSphere3d BoundingSphere(Vector3d center)
        {
            double radiusSqr = 0;
            foreach (var vertex in Vertices())
            {
                double distSqr = center.DistanceSquared(vertex);
                radiusSqr = radiusSqr < distSqr ? distSqr : radiusSqr;
            }
            return new ImplicitSphere3d(center, Math.Sqrt(radiusSqr));
        }
        ...
        public DMesh3(DMesh3 toCopy, bool bCompact = false, bool bWantNormals = true, bool bWantColors = true, bool bWantUVs = true)
        {
            if (bCompact)
                CompactCopy(toCopy, bWantNormals, bWantColors, bWantUVs);
            else
                Copy(toCopy, bWantNormals, bWantColors, bWantUVs);
            CloneCaches(toCopy);
            this.PartName = toCopy.PartName;
            this.InternalID = toCopy.InternalID;
            this.UUID = toCopy.UUID;
        }

        private void CloneCaches(DMesh3 toCopy)
        {
            if (toCopy.timestamp == toCopy.convexShadowTimeStamp)
            {
                _convexShadow = toCopy._convexShadow;
                convexShadowTimeStamp = timestamp;
            }
            if (toCopy.timestamp == toCopy.shadowTimeStamp)
            {
                _shadow = toCopy._shadow;
                shadowTimeStamp = timestamp;
            }
            if (toCopy.timestamp == toCopy.geometryInfoTimeStamp && toCopy.ShapeTimestamp == toCopy.geometryInfoShapeTimeStamp)
            {
                _geometryInfo = toCopy._geometryInfo?.Clone();
                geometryInfoTimeStamp = timestamp;
                geometryInfoShapeTimeStamp = shape_timestamp;
            }
            if (toCopy.timestamp == toCopy._centroidBoundingSphereTimeStamp)
            {
                _centroidBoundingSphereTimeStamp = timestamp;
                _boundingSphereCentroid = new ImplicitSphere3d(toCopy._boundingSphereCentroid);                
            }
            if (toCopy.timestamp == toCopy._centroidTimestamp)
            {
                _centroidTimestamp = timestamp;
                _centroid = toCopy._centroid;
            }
        }
        ...
        /// <param name="ignoreMeshErrors">if true, a non-manifold mesh will not be auto-repaired</param>
        ...
        {
            get { return (VertexCount / (double)MaxVertexID + TriangleCount / (double)MaxTriangleID) * 0.5; }
        }
        ...
        public AxisAlignedBox3d BoundsInFrame(Frame3f frame, Func<Vector3d, Vector3d> TransformF = null)
        {
            if (frame.Rotation == Quaternionf.Identity && TransformF == null)
            {
                var bounds = Bounds;
                bounds.Translate(-frame.Origin);
                Debug.Assert(bounds == MeshMeasurements.BoundsInFrame(this, frame));
                return bounds;
            }

            return MeshMeasurements.BoundsInFrame(this, frame, TransformF);
        }

        public AxisAlignedBox3d BoundsFromFrame(Frame3d frame, Func<Vector3d, Vector3d> TransformF = null)
        {
            if (frame.Rotation == Quaterniond.Identity && TransformF == null)
            {
                var bounds = Bounds;
                bounds.Translate(frame.Origin);
                Debug.Assert(bounds == CalcBoundsFromFrame(frame));
                return bounds;
            }

            return CalcBoundsFromFrame(frame, TransformF);
        }

        private AxisAlignedBox3d CalcBoundsFromFrame(Frame3d frame, Func<Vector3d, Vector3d> TransformF = null)
        {
            ...
            if (TransformF == null)
            {
                ...
                {
                    Vector3d v = frame.FromFrameP(item);
                    empty.Contain(ref v);
                }
            }
            else
            {
                foreach (Vector3d item2 in Vertices())
                {
                    Vector3d v2 = TransformF(item2);
                    Vector3d v3 = frame.FromFrameP(ref v2);
        ...
        public MeshType MeshType { get; set; } = MeshType.PART;

        public OpenVectorFormat.Part.Types.GeometryInfo GeometryInfo
        {
            get
            {
                if (_geometryInfo == null || geometryInfoTimeStamp < timestamp)
                {
                    ComputeGeometryInfo();
                    geometryInfoTimeStamp = timestamp;
                }
        ...
        public AxisAlignedBox3d Bounds
        {
            get
            {
                var aabb = CachedBounds;
                if (_geometryInfo != null)
                    _geometryInfo.BuildHeightInMm = aabb.Max.z;
                return aabb;
            }
        }

        /// <summary>
        /// Function returns the quantity of partial parts that result from a part when it has received an offset as a 3D part.
        /// </summary>
        /// <param name="offset"></param>
        /// <param name="shellResolution"></param>
        /// <returns></returns>

        public DMesh3[] GetOffsetParts(double offset, int shellResolution)
        {
            var offsetMesh = GetOffsetMesh(offset, shellResolution);
            var meshes = MeshConnectedComponents.Separate(offsetMesh);
            var G3meshes = new DMesh3[meshes.Length];

            for (int i = 0; i < meshes.Length; i++)
            {
                var g3SharpMesh = new DMesh3(meshes[i]);
                g3SharpMesh.PartName = "submesh " + i.ToString() + "_Offset " + offset.ToString();
                G3meshes[i] = g3SharpMesh;
            }

            return G3meshes;
        }

        private void ComputeGeometryInfo()
        {
            //get geometry Info
            if (_geometryInfo == null || geometryInfoShapeTimeStamp < this.ShapeTimestamp)
            {
                var volArea = VolumeArea();
                _geometryInfo = new OpenVectorFormat.Part.Types.GeometryInfo();
                _geometryInfo.SurfaceAreaInMm2 = volArea.area;
                if (IsManifoldAndOriented)
                {
                    //computing the shadow is very expensive and will now only be filled if ComputeShadow is called
                    //the data is also voided by rotations
                    //_geometryInfo.ProjectedSurfaceInMm2 = ComputeShadow().Sum(x => x.Area);
                    _geometryInfo.VolumeInMm3 = volArea.volume;
                }
            }

            _geometryInfo.BuildHeightInMm = Bounds.Max.z;
        }

        public (double volume, double area) VolumeArea()
        {
            var volArea = MeshMeasurements.VolumeArea(this, TriangleIndices(), GetVertex);
            return (volArea.x, volArea.y);
        }

        /// <summary>
        /// Cuts this mesh parallel to x-y plane at heights zStart and zEnd.
        /// Removes all geometry below zStart and above zEnd.
        /// </summary>
        /// <param name="zStart"></param>
        /// <param name="zEnd"></param>
        /// <param name="fillHoles"></param>
        public void ZParallelDoubleCut(double zStart, double zEnd, bool fillHoles = false)
        {
            MeshPlaneCut upperCut = new MeshPlaneCut(this, new Vector3d(0, 0, zEnd), new Vector3d(0, 0, 1));
            upperCut.Cut();
            if (fillHoles) MeshPlaneCutAdvanced.AdvancedHoleFill(upperCut);
            MeshPlaneCut lowerCut = new MeshPlaneCut(this, new Vector3d(0, 0, zStart), new Vector3d(0, 0, -1));
            lowerCut.Cut();
            if (fillHoles)
            {
                MeshPlaneCutAdvanced.AdvancedHoleFill(lowerCut);
                new MeshAutoRepair(this).Apply();
            }
            else
            {
                ShadowGenerator.AddCutToG3Mesh(this, lowerCut);
                ShadowGenerator.AddCutToG3Mesh(this, upperCut);
            }
            timestamp++;
            shape_timestamp++;
        }
        ...
        /// <summary>
        /// Computes and returns the convex hull of the shadow (projection onto x-y-plane) of this mesh. 
        /// </summary>
        /// <returns>the convex shadow</returns>
        public Polygon2d ComputeConvexShadow(double epsilon = ConvexHull2.DefaultConvexHullTolerance)
        {
            if (_convexShadow == null || convexShadowTimeStamp < timestamp)
            {
                _convexShadow = this.ConvexShadow(epsilon);
                convexShadowTimeStamp = this.Timestamp;
            }
            return _convexShadow;
        }

        /// <summary>
        /// Computes and returns the shadow (projection onto x-y-plane) of this mesh
        /// </summary>
        /// <returns></returns>
        public IList<GeneralPolygon2d> ComputeShadow()
        {
            if (_shadow == null || shadowTimeStamp != timestamp)
            {
                if (cutShadowHint == null)
                {
                    System.Diagnostics.Debug.Assert(this.IsManifoldAndOriented, "shadow compute is only tested for manifold meshes");
                }
                // safe guard for VERY large meshes
                if (this.TriangleCount > RealShadowTriangleCountLimit)
                {
                    _shadow = ShadowGenerator.PiecewiseConvexShadow(this, RealShadowTriangleCountLimit / 100).SortPolygons().ToArray();
                }
                else
                {
                    _shadow = ShadowGenerator.GetShadowPolygons2d(this);
                }
                shadowTimeStamp = timestamp;
                if (_shadow == null) throw new ApplicationException("unable to compute shadow, mesh structure might be non-manifold");
            }
            if (_geometryInfo != null) _geometryInfo.ProjectedSurfaceInMm2 = _shadow.Sum(x => x.Area);
            return _shadow;
        }
        ...
        public void RepairMesh()
        {
            if (!IsManifoldAndOriented)
            {
                ILogging.Logger?.Warning($"mesh {PartName} is not manifold and will be repaired");

                var MeshAutoRepair = new MeshAutoRepair(this);
                //MeshAutoRepair.MinEdgeLengthTol = 0.000001;
                //MeshAutoRepair.RepairTolerance = 0.0001;
                MeshAutoRepair.Apply();
                var temp = PartName;
                PartName = temp;
                if (!IsManifoldAndOriented)
                {
                    ILogging.Logger?.Error($"mesh {PartName}: automatic repair failed");
                }
            }
        }

        public bool IsManifoldAndOriented => this.CachedIsClosed;

        #region meshTransformations

        /// <summary>
        /// Rotates the mesh around its centroid with the given euler angles (in degrees).
        /// </summary>
        /// <param name="pitch">rot x</param>
        /// <param name="yaw">rot y</param>
        /// <param name="roll">rot z</param>
        public void Rotate(float pitch, float yaw, float roll)
        {
            if (pitch != 0 || yaw != 0 || roll != 0)
            {
                Quaternionf rotation = TypeConversions.EulerDToQuaternion(pitch, yaw, roll);
                Vector3d centroid = Centroid;

                Frame3f rotationFrame = Frame3f.Identity;
                rotationFrame.RotateAround((Vector3f)centroid, rotation);

                MeshTransforms.FromFrame(this, rotationFrame);
                timestamp++;
            }
        }

        public void TransformFromFrame(Frame3f frame)
        {
            MeshTransforms.FromFrame(this, frame);
            timestamp++;
        }

        public void TransformFromFrame(Frame3d frame)
        {
            MeshTransforms.FromFrame(this, frame);
            timestamp++;
        }
        ...
        {
            MeshTransforms.Translate(this, x, y, z);
            timestamp++;
        }
        #endregion

        /// <summary>
        /// reduces the triangle count of this mesh by the given percent
        /// i.e. 20% as input => mesh will retain 80% resolution
        /// </summary>
        /// <param name="triCountRedPercentage"></param>
        public static void MeshReduceByPercent(DMesh3 mesh, int triCountRedPercentage)
        {
            if (triCountRedPercentage <= 0) return;
            if (triCountRedPercentage >= 100) throw new ArgumentException("cannot reduce mesh by 100 or more percent");

            int targetTriangles = (mesh.TriangleCount * (100 - triCountRedPercentage)) / 100;
            Reducer r = new Reducer(mesh);
            r.ReduceToTriangleCount(targetTriangles);
            mesh.CompactInPlace();
        }

        public void WriteMeshToFile(string path, DMesh3 mesh)
        {
            StandardMeshWriter.WriteMesh(path, mesh, WriteOptions.Defaults);
        }
        ...
        /// </summary>
        /// <param name="offset"></param>
        /// Each vertex is shifted absolutely by the offset.
        /// <param name="numCells"></param>
        /// <returns></returns>
        public DMesh3 GetOffsetMesh(double offset, int numCells = 128)
        {
            var shellMeshImplicit = this.ToImplicitFunctionDGridTriL(numCells / 2, offset);
            ImplicitOffset3d B = new ImplicitOffset3d() { A = shellMeshImplicit, Offset = -offset };
            var offsetMesh = new DMesh3(B.ToMeshMarchingCubes(numCells));
            return offsetMesh;
        }

        public DMesh3 GetHomogeinShellWall(double shell_thickness, int shell_mesh_resolution = 128)
        {
            var shellMeshImplicit = this.ToImplicitFunctionDGridTriL(shell_mesh_resolution / 2, shell_thickness);
            ImplicitOffset3d B = new ImplicitOffset3d() { A = shellMeshImplicit, Offset = -shell_thickness };
            var shell = new ImplicitDifference3d()
            {
                A = shellMeshImplicit,
                B = B,
            };
            var shellMesh = shell.ToMeshMarchingCubes(shell_mesh_resolution);

            return new DMesh3(shellMesh);
        }

        /// <summary>
        /// The function scales the DMesh3 so that the maximum bounding box length corresponds to the scale factor scale absolute.
        /// </summary>
        /// <param name="scale"></param>
        public void Scale(double scale)
        {
            MeshTransforms.Scale(this, scale / this.CachedBounds.MaxDim);
        }

        /*
        /// <summary>
        /// Returns Features that disappear when offsetting by halfHoleSize. Usually those are "drilled" holes with the given radius and some meshes that can be considered noise
        /// </summary>
        /// <param name="holeRadius"></param>
        /// <param name="numCells">higher means more accurate results but more computing</param>
        /// <param name="filterByVolume">optional parameter that removes meshes smaller than the given value from the resultset</param>
        /// <param name="pathOut">optional: only pass parameter if you want to export intermediate results</param>
        /// <returns></returns>
        public List<DMesh3> GetHolesBySize(double holeRadius, int numCells, double? filterByVolume = null, string pathOut = null)
        {
            var outset = this.GetOffsetMesh(-holeRadius, numCells);
            outset.PartName = this.PartName;

            if (pathOut != null)
                outset.Export(pathOut, DAPModel.FileFormats.STL, suffix: "outset");

            var inset = outset.GetOffsetMesh(holeRadius + 0.2, numCells);
            inset.PartName = this.PartName;

            if (pathOut != null)
                inset.Export(pathOut, DAPModel.FileFormats.STL, suffix: "inset");

            var shellMeshImplicit = inset.this.ToImplicitFunctionDGridTriL(numCells / 2, holeRadius);
            var originalMeshImplicit = this.this.ToImplicitFunctionDGridTriL(numCells / 2, holeRadius);

            var implicitHolesVolumesMesh = new ImplicitDifference3d() { A = shellMeshImplicit, B = originalMeshImplicit };
            var holesVolumesMesh = new DMesh3(implicitHolesVolumesMesh.ToMeshMarchingCubes(numCells));

            if (pathOut != null)
                holesVolumesMesh.Export(pathOut, DAPModel.FileFormats.STL, suffix: "result");

            var result = holesVolumesMesh.this.SeparatePartsIntoSingleMeshes($"{this.PartName}_hole_", filterByVolume);
