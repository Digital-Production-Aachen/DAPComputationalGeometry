...
namespace g3
{
    public class PolyLine2d : IEnumerable<Vector2d>
    {
        ...
        private int boundsCachedTimeStamp = -1;
        ...
        public AxisAlignedBox2d GetBounds() => Bounds;
        ...
        public IEnumerable<(Vector2d intersectionPoint, double t)> FindIntersections(Polygon2d polygon, double startIntervalThresh = MathUtil.ZeroTolerance, double dotThreshold = MathUtil.ZeroTolerance, double intervalThresh = 0)
        {
            ...
            {
                Segment2d seg = new Segment2d(vertices[i], vertices[i + 1]);
                if (i == 0) { seg.Center = seg.Center + seg.Direction * startIntervalThresh / 2; seg.Extent -= startIntervalThresh / 2; }
                if (i == vertices.Count - 1) { seg.Center = seg.Center - seg.Direction * startIntervalThresh / 2; seg.Extent -= startIntervalThresh / 2; }
                foreach (Segment2d oseg in polygon.SegmentItr())
                {
                    if (seg.Intersects(oseg, out double t1, out double t2, dotThreshold, intervalThresh))
                    {
                        yield return (seg.PointAt(t1), tSum + t1);
                    }
                }
                tSum += seg.ArcLength;
            }
        ...
        public PolyLine2d Translate(Vector2d translation)
        {
            if (VertexCount < 12)
            {
                int N = vertices.Count;
        ...
        public PolyLine2d Rotate(Matrix2d rotation, Vector2d origin)
        {
            if (VertexCount < 12)
            {
                int N = vertices.Count;
